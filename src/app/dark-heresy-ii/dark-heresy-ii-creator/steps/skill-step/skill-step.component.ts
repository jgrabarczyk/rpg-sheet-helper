...
export class SkillStepComponent {
  ...
  @Input() set skillsToPick(skillsToPick: DHII_SkillName[][]) {
    this.skillsToPick_ = skillsToPick.map(skills =>
      mapStringArrayToSelectOptionArray(skills)
    );
    this.skillsToPick_.forEach(() =>
  ...
  @Output() updateSkills: EventEmitter<DHII_SkillName[]> = new EventEmitter<
    DHII_SkillName[]
  >();
