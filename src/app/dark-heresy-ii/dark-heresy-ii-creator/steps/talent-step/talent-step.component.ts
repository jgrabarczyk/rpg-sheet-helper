...
export class TalentStepComponent {
  ...
  @Input() set talentsToPick(talentsToPick: DHII_TalentName[][]) {
    this.talentsToPick_ = talentsToPick.map(talents =>
      mapStringArrayToSelectOptionArray(talents)
    );
    this.talentsToPick_.forEach(() =>
  ...
  @Output() updateTalents: EventEmitter<DHII_TalentName[]> = new EventEmitter<
    DHII_TalentName[]
  >();
