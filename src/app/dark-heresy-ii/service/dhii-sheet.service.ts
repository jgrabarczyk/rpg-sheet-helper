...
export class DHII_SheetService {
  protected attributesSubject$: BehaviorSubject<DHII_Attribute[]> =
    new BehaviorSubject<DHII_Attribute[]>(structuredClone(DHII_ATTRIBUTE_LIST));

  protected skillSubject$: BehaviorSubject<DHII_Skill[]> = new BehaviorSubject<
    DHII_Skill[]
  >(
    structuredClone(DHII_SKILLS_LIST).map((skill) => ({
      ...skill,
      value:
        this.attributesSubject$.value.find(
          (attribute) => attribute.name === skill.basedOn
        )?.value ?? 0,
    }))
  );

  attributes$: Observable<DHII_Attribute[]> =
  ...
  updateAttribute(changedAttribute: DHII_Attribute) {
    const attributes = this.attributesSubject$.value;
    attributes.find((attribute) => attribute.name === changedAttribute.name)
      ?.value === changedAttribute.value;
  ...
  changeSkill(changedSkill: DHII_Skill) {
    const attribute = this.attributesSubject$.value.find(
      (attribute) => attribute.name === changedSkill.basedOn
    )!;
    const skills = this.skillSubject$.value;

    skills.find((skill) => skill.name === changedSkill.name)!.value =
      this.calculateSkillValue(changedSkill, attribute);

    this.skillSubject$.next(skills);
  }

  private updateSkillsBasedOnAttribute(changedAttribute: DHII_Attribute) {
    const skills = this.skillSubject$.value.filter(
