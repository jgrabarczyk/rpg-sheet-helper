...
describe('SheetComponent', () => {
  ...
  describe('check "/sheet/:prefix/:saveName" path ', () => {
    ...
    it(`Should initialize component and save character as "${newSaveName}"`, async () => {
      //click save button
      const saveCharacterButton: MatButtonHarness =
        await getButtonHarnessWithSelector(componentLoader, 'saveCharacter');
      ...
      const eqTab: MatTabHarness[] =
        await componentLoader.getAllHarnesses(MatTabHarness);
    ...
    describe('test equipment', () => {
      it('should add all weapons with "Laspistol" in name', async () => {
        const addEquipmentButton: MatButtonHarness =
          await getButtonHarnessWithSelector(componentLoader, 'addItems');

        await addEquipmentButton.click();

        const equipmentDialog: MatDialogHarness =
          await rootLoader.getHarness(MatDialogHarness);
        ...
        const weaponFilterQueryInput: MatInputHarness =
          await weaponTab.getHarness(MatInputHarness);
        await weaponFilterQueryInput.setValue(weaponFilterQuery);

        // //mark all filtered result as checked
        const [firstWeapon, secondWeapon] =
          await weaponTab.getAllHarnesses(MatCheckboxHarness);
        ...
        //save added items
        const addItemsButton: MatButtonHarness =
          await equipmentDialog.getHarness(
      ...
      it('should add empty equipment list', async () => {
        ...
        const backpacks: BackpackHarness[] =
          await eqPanel.getAllHarnesses(BackpackHarness);
      ...
      it('should add one item from every type (Backapck Items, Cybernetic, Melee Weapons, Thrown Weapons, Ranged Weapons, Armours, Force Field, )', async () => {
        ...
        const [
          ...
          weaponsTab,
          armoursTab,
        ]: MatTabHarness[] = await equipmentDialog.getAllHarnesses(
          MatTabHarness.with()
        );

        await backpackTab.select();
        const backpackFilter: MatInputHarness =
          await backpackTab.getHarness(MatInputHarness);
        await backpackFilter.setValue('Chrono');
        const backpackItem: MatCheckboxHarness =
          await backpackTab.getHarness(MatCheckboxHarness);
        await backpackItem.check();

        await cyberneticsTab.select();
        const cyberneticsFilter: MatInputHarness =
          await cyberneticsTab.getHarness(MatInputHarness);
        await cyberneticsFilter.setValue('Bionic Heart');
        const cyberneticsItem: MatCheckboxHarness =
          await cyberneticsTab.getHarness(MatCheckboxHarness);
        await cyberneticsItem.check();

        await weaponsTab.select();
        const weaponsFilter: MatInputHarness =
          await weaponsTab.getHarness(MatInputHarness);
        await weaponsFilter.setValue('Force Staff');
        let weaponsItem: MatCheckboxHarness =
          await weaponsTab.getHarness(MatCheckboxHarness);
        ...
        //add standard armour
        const armoursFilter: MatInputHarness =
          await armoursTab.getHarness(MatInputHarness);
        await armoursFilter.setValue('Light Power Armour');
        let armoursItem: MatCheckboxHarness =
          await armoursTab.getHarness(MatCheckboxHarness);
        ...
        const [
          ...
          forceFields,
          backpack,
        ]: BackpackHarness[] = await eqPanel.getAllHarnesses(BackpackHarness);
        const meleeWeaponItems: TestElement[] =
          await meleeWeapons.getTableRows();
        const rangedWeaopnItems: TestElement[] =
          await rangedWeapons.getTableRows();
        const thrownWeaopnItems: TestElement[] =
          await thrownWeapons.getTableRows();
        ...
        expect(await forceFieldItems[0].text())
          .withContext('should contian valid force field description')
          .toContain(
        ...
        expect(await backpackItems[0].text())
          .withContext('should contian valid description')
          .toContain('Keeps time.');
        expect(await backpackItems[1].text()).toContain('Bionic Heart');
        expect(await backpackItems[1].text())
          .withContext('should contian valid description')
          .toContain('+1 AP to Body, grants Sprint talent.');
      });
    });
    ...
    describe('should test attribute', () => {
      it('should increase "Weapon Skill" attribute to max (from 0 to 25)', async () => {
        ...
        const weaponSkillAttributeItem: MatListItemHarness =
          await attributeCard.getHarness(
        ...
        const increaseAttributeLevelButton: MatButtonHarness =
          await weaponSkillAttributeItem.getHarness(
            MatButtonHarness.with({
              selector: '[name="increaseAttributeLevel"]',
            })
          );
        const decreaseAttributeLevelButton: MatButtonHarness =
          await weaponSkillAttributeItem.getHarness(
            MatButtonHarness.with({
              selector: '[name="decreaseAttributeLevel"]',
            })
          );
      ...
      it('should roll "Weapon Skill" attribute with 1% chance to success', async () => {
        ...
        const rollMenu: MatMenuHarness =
          await wsItem.getHarness(MatMenuHarness);
        const rollMenuItems: MatMenuItemHarness[] = await rollMenu.getItems({
          text: '0',
        });
        ...
        const rollItem: MatListItemHarness =
          await diceLoggerCard.getHarness(MatListItemHarness);
    ...
    describe('should test skill', () => {
      it('should increase "Trade" skill up to max (from 0 to 30) and roll test', async () => {
        ...
        const increaseSkillLevelButton: MatButtonHarness =
          await tradeSkillItem.getHarness(
            MatButtonHarness.with({ selector: '[name="increaseSkillLevel"]' })
          );
        const decreaseSkillLevelButton: MatButtonHarness =
          await tradeSkillItem.getHarness(
        ...
        const tradeSkillValue: MatInputHarness =
          await tradeSkillItem.getHarness(MatInputHarness);
        expect(await tradeSkillValue.getValue()).toBe('30');

        const quickRollButton: MatButtonHarness =
          await tradeSkillItem.getHarness(
    ...
    it('should delete current character and navigate to "/load" path ', async () => {
      const deleteCharacterButton: MatButtonHarness =
        await getButtonHarnessWithSelector(componentLoader, 'deleteCharacter');
      await deleteCharacterButton.click();

      const confirmDialog: MatDialogHarness =
        await rootLoader.getHarness(MatDialogHarness);
